...
IF NOT "%file%"=="" (
    if defined SET_POSTGRES_BIN (
        for /f "tokens=*" %%i in (%file%) do (
			...
			"%SET_POSTGRES_BIN%\psql.exe" -v gtin='!cod!' -h localhost -p 5432 -U postgres --no-password --dbname set_mark -x -f "C:\query_marks.sql"> NUL
    ...
    if defined SET_POSTGRES_BIN (
        "%SET_POSTGRES_BIN%\psql.exe" -v gtin='!code!' -h localhost -p 5432 -U postgres --no-password --dbname set_mark -x -f "C:\query_marks.sql" > NUL
    ) else (
        echo "Переменная среды SET_POSTGRES_BIN не определена в системе"
    )
) ELSE (
